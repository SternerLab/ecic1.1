...
datList <- list()
set.seed(221)
# generate data draws of different sample sizes
for(i in 1:nsLen)
{
  tempN <- ns[i]
...
for(i in 1:nsLen) # i indexes sample sizes
{
  ...
  for(j in 1:MLen) # j indexes the assumed true probability 
  {
    ...
    ICsSimDat2[[i]][[j]] <- ICComputations(simDat2List[[i]][[j]],M,MLen,N2,
                                           "NormalNegLL",MNames)
    rownames(ICsSimDat1[[i]][[j]]) <- paste("-LL Under ",MNames,sep="")
    rownames(ICsSimDat2[[i]][[j]]) <- paste("-LL Under ",MNames,sep="")
  }
...
# make barplots for the percentage of the time each model was chosen 
# as best for each sample size
par(mfrow = c(1, 2)) 
for(i in 1:nsLen)
{
