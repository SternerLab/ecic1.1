# This file runs the same program as in Norm_Asymp_WithMLE
# except that the only sample size considered is 500, the model set is reduced to
# $\mathcal{M}=\{N(\mu,1),N(\mu,\sigma)\}$, and a a multiplicative factor of 10
# applied to the penalty term on the BIC calculations.
# The objective was to observe if the rate of model
# selection and correct model selection increase to $1$ asymptotically, and it appears that
# which appears to be true. The adjustment to the penalty term for the BIC
# was used as a proxy for increasing the sample size to larger values (say 10K)
...
trueModel <- "N(0.5,1)"
MNames <- c("N(mu,1)","N(mu,sigma)") #c("N(0,1)","N(mu,1)","N(mu,sigma)")
...
for(i in 1:nsLen)
{
  ...
  # MLE's for N(mu,1) 
  MLEList[[i]][[1]] <- rbind(tempMeanMLEs,1)
  rownames(MLEList[[i]][[1]]) <- c("MLEMean","fixedSig")
  # MLE's for N(mu,sigma)
  MLEList[[i]][[2]] <- rbind(tempMeanMLEs,tempSigMLEs)
  rownames(MLEList[[i]][[2]]) <- c("MLEMean","MLESig")
  colnames(MLEList[[i]][[1]]) <- colnames(MLEList[[i]][[2]]) <- 
    colnames(MLEList[[i]][[2]]) <- paste("Draw",1:noDraws,sep="")
...
for(i in 1:nsLen)
{
  ICComps[[i]] <- ICComputations(datMat=datList[[i]],M=M,MLen=MLen,
                                 noDraws=noDraws,
                                 ICType="BICNormLP",MNames=MNames)
}
