...
# for a model set of three normal distributions with means
# 2.5, 4, and 5 (with known variance=1)
# We simulate 2 scenarios:
# Scenario #1: The true mean is 4 
# Scenario #2: The true mean is 4.3
...
# the first list will possess data generated from a normal distribution
# with mu=4 and the second with mu=4.3
...
for(i in 1:nsLen)
{
  ...
  tempDraws <- rnorm(n=tempN*noDraws,mean=trueMu1,sd=sig)
  ...
  # draw from a normal dist with current n & p
  tempDraws <- rnorm(n=tempN*noDraws,mean=trueMu2,sd=sig)
  # convert vector of draws into a matrix of multiple draws
  tempDraws <- matrix(data=tempDraws,nrow=tempN,ncol=noDraws)
  # store matrix as the draws that are each of sample size tempN 
...
names(datList1) <- paste("True mu=",trueMu1,",n=",ns,sep="")
names(datList2) <- paste("True mu=",trueMu2,",n=",ns,sep="")
...
for(i in 1:nsLen)
{
  ...
  for(j in 1:MLen)
  {
    tempICs <- -1*apply(X=datList1[[i]],MARGIN=2,FUN=function(x) LLNorm(x,mu=M[j]))
    tempMat[j,] <- tempICs
  }
  ICComps[[i]] <- tempMat
...
# note 1 corresponds to mu=0.5, 2 to mu=0.65, and 3 to 0.75
...
for(i in 1:nsLen) # i indexes sample sizes,
{
  ...
  for(j in 1:MLen) #  j indexes the assumed true mean
  {
    ...
    simDat2List[[i]][[j]] <- rnorm(n=tempN*N2,mean=M[j],sd=sig)
    simDat2List[[i]][[j]] <- matrix(data=simDat2List[[i]][[j]],nrow=tempN,ncol=N2)
    colnames(simDat2List[[i]][[j]]) <- paste("Draw",1:N2,sep="")
  }
  names(simDat1List[[i]]) <- paste("True mu=",M,sep="")
  names(simDat2List[[i]]) <- paste("True mu=",M,sep="")
}
...
for(i in 1:nsLen) # i indexes sample sizes
{
  ...
  for(j in 1:MLen) # j indexes the assumed true probability 
  {
    ...
    {
      tempICs1 <- -1*apply(X=tempDraws1,MARGIN=2,FUN=function(x) LLNorm(x,mu=M[k]))
      tempICs2 <- -1*apply(X=tempDraws2,MARGIN=2,FUN=function(x) LLNorm(x,mu=M[k]))
      tempMat1[k,] <- tempICs1
      tempMat2[k,] <- tempICs2
    }
    ICsSimDat1[[i]][[j]] <- tempMat1
    ICsSimDat2[[i]][[j]] <- tempMat2
    rownames(ICsSimDat1[[i]][[j]]) <- paste("-LL Under mu=",M,sep="")
    rownames(ICsSimDat2[[i]][[j]]) <- paste("-LL Under mu=",M,sep="")
  }
  names(ICsSimDat1[[i]]) <- paste("True mu=",M,sep="")
  names(ICsSimDat2[[i]]) <- paste("True mu=",M,sep="")
}
...
for(i in 1:nsLen) # i indexes sample size
{
  ...
  names(minICList[[i]]) <- paste("True mu=",M,sep="")
}
...
for(i in 1:nsLen) # indexes the sample size
{
  ...
  rownames(tempMat) <- paste("true mu=",M,sep="")
  colnames(tempMat) <- paste("% of time mu=",M," observed best ",sep="")
...
for(i in 1:nsLen) # index the sample size
{
  ...
  names(DGOFList[[i]]) <- paste("True mu=",M,sep="")
}
...
for(i in 1:nsLen)
{
  ...
  for(j in 1:MLen)
  {
    tempICs <- -1*apply(X=datList2[[i]],MARGIN=2,FUN=function(x) LLNorm(x,mu=M[j]))
    tempMat[j,] <- tempICs
  }
  ICComps[[i]] <- tempMat
  # columns are draws and rows are the IC evaluated under different parameter values
